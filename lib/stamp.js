...
export async function stampSignature(options) {
  ...
  const image = embedded === undefined
    ? undefined
    : embedded.kind === 'png'
      ? await doc.embedPng(embedded.bytes)
      : await doc.embedJpg(embedded.bytes)
  ...
  return {
    ...
    note: 'This is a VISUAL signature (ink on the page). It carries no cryptographic guarantee; use pdf_sign_digital for a verifiable signature.'
  }
}
